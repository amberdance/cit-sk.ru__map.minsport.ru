...
const validateData = async (responseData, payload) => {
  ...
  if (typeof responseData === 'string') { return Promise.reject('Cannot find condition to handle request data') }

  if (Object.keys(responseData).length == 1 && responseData.status) { return payload }
  ...
  if (responseData instanceof Object && 'data' in responseData) { return responseData.data }
